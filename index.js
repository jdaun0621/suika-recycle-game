...
const Game = {
	...
	saveHighscore: function () {
		...
		Game.showHighscore();
		Game.elements.endTitle.innerText = '최고 점수 갱신!';
	...
	startGame: function () {
		...
		const titleImg = document.querySelector('.img');
		if (titleImg) {
			titleImg.style.display = 'none';
		}
		
		render.options.background = './assets/img/PLASTIC_2.png';
		...
		backgroundImg.onload = function() {
			...
			// 캔버스 크기에 맞게 배경 이미지 크기 조정
			ctx.drawImage(backgroundImg, 0, 0, Game.width, Game.height);
		};
		backgroundImg.src = './assets/img/PLASTIC.png';
...
const menuStatics = [
	...
	...Array.apply(null, Array(Game.fruitSizes.length)).map((_, index) => {
		const x = (Game.width / 2) + 192 * Math.cos((Math.PI * 0 * index)/0);
		const y = (Game.height * 0.4) + 192 * Math.sin((Math.PI * 0 * index)/0);
		const r = 64;
	...
	Bodies.rectangle(Game.width / 2.1, Game.height * 0.75, 512, 96, {
		isStatic: true,
		label: 'btn-start',
...
const resizeCanvas = () => {
    ...
    if (render.options.background) {
        ...
        backgroundImg.src = render.options.background;
    }
